...
public class Reader {
	...
	static LinkedList<Walls> lines = new LinkedList<Walls>();
	static LinkedList<Walls> space = new LinkedList<Walls>();
	static HashMap<String, LinkedList> buildings = new HashMap<>();
	static HashMap<String, LinkedList> buildingPictures = new HashMap<>();
	static String name;
	...
	//Uses the data in the tokens to substract the necesary values
	//Of each wall
	public static void reader() {
		...
		while(i < arr.length) {
			if(arr[i].equals(" Building")){
				...
				i++;
				for(int a = 0; a < walls.size(); a+=6) {
					lines.add(new Walls(walls.get(a),walls.get(a+1),walls.get(a+2),walls.get(a+3),walls.get(a+4), walls.get(a+5)));
				}
